...
function* watchLogin() {
  yield takeEvery('LOG_IN_REQUEST', logIn); //로그인이라는 액션이 실행될 때까지 기다리겠다. 액션이 실행되면 login함수 실행, 이벤트 리스너 같은 역할
}

function* watchLogOut() {
  yield takeEvery('LOG_OUT_REQUEST'); //take의 치명적인 단점은 1회용.. 1번 밖에 안 받음.. 해결은. while로 true로 감싸거나, 아니면 takeEvery.
